    ...
    vector<Token> tokenize() {
        ...
        while (!eof()) {
            ...
            if (isAlpha(c)) {
                ...
                // C# allows @identifier to use reserved words as identifiers; handle basic '@' prefix:
                if (word == "@" && !eof() && isAlpha(peek())) {
                    size_t atStart = start;
                    // read the identifier after '@'
                    get(); // consume '@'
                    while (!eof() && (isAlphaNum(peek()) || peek() == '_')) get();
                    word = input.substr(atStart, pos - atStart);
                }
